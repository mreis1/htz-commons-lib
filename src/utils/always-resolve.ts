...
export type AlwaysResolvePromiseCall<T> = Promise<T> | AlwaysResolvePromiseFn<T>;
          ...
          switch (typeof v) {
               case 'function':
                    try {
                         ...
                         if (o instanceof Promise) {
                              o.then((data) => resolve({ err: null, data })).catch((err) => resolve({ err, data: null }));
                         } else {
                              !opts?.promiseOnly ? resolve({ err: null, data: o }) : resolve({ err: new TypeError('INVALID_PROMISE'), data: null });
                         }
                    } catch (err) {
               ...
               default:
                    if (v instanceof Promise) {
                         v.then((data) => resolve({ err: null, data })).catch((err) => resolve({ err, data: null }));
                    } else {
                         !opts?.promiseOnly ? resolve({ err: null, data: v }) : resolve({ err: new TypeError('INVALID_PROMISE'), data: null });
                    }
          }
     });
}
